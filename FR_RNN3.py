...
class FiringRateRNNCell(LayerRNNCell):
  ...
  def call(self, inputs, state):
    ...
    noise = random_ops.random_normal(array_ops.shape(state),mean = 0.0,  stddev=hiddenNoiseStd)
    state  = state + noise
...
def VPDATAtrainingN1(BatchSize = 1000,ratio = .25):
  """
  ...
  with 0.2 possibility of only 1 stimulus, left is 2 stimulus. all the delay period is random
  Returns:
  ...
  for i in range(BatchSize):
    # p = np.random.rand()
    # if  p < 0.2:
    #   S1Start = np.int64(np.random.uniform(0.1*TimeSteps))
    #   S1End = np.int64(S1Start + np.random.uniform(0.4*TimeSteps))
    #   S2Start = np.int64(S1End + np.random.uniform(0.1*TimeSteps))
    #   S2End = np.int64(S2Start + np.random.uniform(0.4*TimeSteps))
    #   S1Start = np.int64(np.floor(np.random.uniform(0.2*TimeSteps)))
    # if p<0.4:
    # # p = np.random.rand()
    #   S1Start = np.int64(np.random.uniform(0.2*TimeSteps))
    #   S1End = np.int64(S1Start + np.random.uniform(0.3*TimeSteps))
    #   S2Start = np.int64(S1End + np.random.uniform(0.2*TimeSteps))
    #   S2End = np.int64(S2Start + np.random.uniform(0.3*TimeSteps))
    # elif p<0.9:
    #   S1Start = np.int64(np.random.uniform(0.2*TimeSteps))
    #   S1End = np.int64(S1Start + np.random.uniform(0.2*TimeSteps))
    #   S2Start = np.int64(S1End + np.random.uniform(0.2*TimeSteps))
    #   S2End = np.int64(S2Start + np.random.uniform(0.2*TimeSteps))
    # else:
      # = np.int64(S2Start + np.random.uniform(0.1*TimeSteps))
    # if np.random.rand()>0.5:
    S1Angle = np.random.rand()*np.pi
    delta = np.random.uniform(0,ratio)*np.pi
...
def VPDATAtestN2(BatchSize = 1000,a1 = 53.,a2 = 50.):
  ...
  return X/x, Y
def VPDATAtestN3(BatchSize = 180):
  
  X = np.zeros((BatchSize,TimeSteps,CELLNUM),dtype = np.float32)
  Y = np.zeros((BatchSize,TimeSteps,5),dtype  = np.float32)
  for i in range(BatchSize):
    S1Angle = i/180.*np.pi
    S2Angle = S1Angle + 3./180.*np.pi
    X[i,S1Start:S1End,:] = np.tile(V1CellFR(StimAngle = S1Angle),(S1End - S1Start,1))
    X[i,S2Start:S2End,:] = np.tile(V1CellFR(StimAngle = S2Angle),(S2End - S2Start,1))
    Y[i, GoCue:TimeSteps, :] = np.tile([np.sin(S1Angle),np.cos(S1Angle),np.sin(S2Angle),np.cos(S2Angle),1],(TimeSteps - GoCue,1))
    x = np.max(X)
  return X/x, Y
def VPDATAtestN4(BatchSize = 180):
  ...
  for i in range(BatchSize):
    ...
    X[i,S1Start:S1End,:] = np.tile(V1CellFR(StimAngle = S1Angle),(S1End - S1Start,1))
    X[i,S2Start:S2End,:] = np.tile(V1CellFR(StimAngle = S2Angle),(S2End - S2Start,1))
    Y[i, GoCue:TimeSteps, :] = np.tile([np.sin(S1Angle),np.cos(S1Angle),np.sin(S2Angle),np.cos(S2Angle),0],(TimeSteps - GoCue,1))
    x = np.max(X)
  return X/x, Y
